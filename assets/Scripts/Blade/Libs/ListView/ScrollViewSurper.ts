...
export class ScrollViewSurper extends cc.EventTarget {
    ...
    protected scroll_to_end_cb: Function;
    protected chat_auto_scrolling: boolean;
    protected auto_scrolling: boolean;
    protected render_cb: Function;
    ...
    private moreNum: number;
    private initContentPos: number;
    private initContentLength: number;
    ...
    constructor(params: ScrollViewParams) {
        ...
        this.chat_auto_scrolling = params.chat_auto_scrolling || false;
        this.auto_scrolling = params.auto_scrolling || false;
        this.moreNum = params.moreNum || 2;
        this.render_cb = params.render_cb;
        ...
        if (this.chat_auto_scrolling) {
            this.auto_scrolling = true;
        }

        if (this.dir == ScrollDirection.Vertical) {
            this.set_content_size(this.width, 0);
            this.initContentPos = this.content.y;
            this.initContentLength = this.height;
        }
        else {
            this.set_content_size(0, this.height);
            this.initContentPos = this.content.x;
            this.initContentLength = this.width;
        }
        ...
        blade.ticker.onTick(this.onUpdate, this);
        // console.log("constructor", this.mask.width, this.mask.height, this.scrollview.node.width, this.scrollview.node.height, this.content.width, this.content.height);
    }
    ...
    protected handle_release_logic() {
        if (this._touchBeganPosition == null || this._touchEndPosition == null) {
            return;
        }
    ...
    protected on_scrolling() {
        ...
        if (this.dir == ScrollDirection.Vertical) {
            ...
            length = this.content.height;
        }
        else {
            pos = this.content.x;
            threshold = this.min_item_width;
            length = this.content.width;
        }

        if (this.chat_auto_scrolling) {
            let offset1 = Math.abs(pos - this.initContentPos);
            let offset2 = Math.abs(length - this.initContentLength);
            if (length - this.initContentLength > 0 && Math.abs(offset1 - offset2) < 5) {
                this.auto_scrolling = true;
            }
            else {
    ...
    protected render() {
        ...
        if (this.dir == ScrollDirection.Vertical) {
            ...
            start = Math.max(start - 1 - this.moreNum, 0);
            stop = Math.min(this.packItems.length - 1, stop + 1 + this.moreNum);
    ...
    onUpdate() {
        if (this.renderDirty && cc.isValid(this.scrollview.node)) {
            console.log("scrollView, render_from:", this.start_index, this.stop_index);
            this.render_items();
            this.renderDirty = false;
        }
        if (this.isAppend && this.render_cb) {
            this.render_cb();
            this.isAppend = false;
        }
    }
    ...
    private inner_select_item(index: number, is_select: boolean) {
        ...
        if (!packItem) {
            console.warn("inner_select_item index is out of range{", 0, this.packItems.length - 1, "}", index);
            return;
        }
    ...
    private spawn_item(key: string, is_measure = false): ScrollViewItem {
        ...
        let item: ScrollViewItem = null;
        if (pools && pools.length > 0) {
            // item = pools.pop();
            for (let i = pools.length - 1; i >= 0; i--) {
                if (pools[i].node.active == false) {
                    item = pools.splice(i, 1)[0];
        ...
        if (item == null) {
            ...
            item.onInit(key);
            // console.log("spawn_item, key=", key);
        }
        if (!is_measure) {
    ...
    private pack_item(index: number, data: ScrollItemData, rendered: boolean = true): PackItem {
        const item = this.spawn_item(data.key, false);
        const [width, height]: [number, number] = item.onSetData(data.key, data.data, index, true);
        const packItem: PackItem = { x: 0, y: 0, width, height, item, key: data.key, data: data.data, is_select: false, rendered: rendered };
    ...
    private set_content_size(width: number, height: number) {
        ...
        // console.log("ScrollView, set_content_size", width, height, this.content.width, this.content.height);
    }
    ...
    insert_data(index: number, ...datas: ScrollItemData[]) {
        ...
        if (index < 0 || index > this.packItems.length) {
            console.warn("insert_data, invalid index", index);
            return;
        }
        let is_append = index == this.packItems.length;
        this.isAppend = is_append;
        let packItems: PackItem[] = [];
        datas.forEach((data, index) => {
            let packItem = this.pack_item(index, data, false);
    ...
    remove_data(index: number, count: number = 1) {
        ...
        if (index < 0 || index >= this.packItems.length) {
            console.warn("remove_data, invalid index", index);
            return;
        }
        if (count < 1) {
    ...
    scroll_to_end() {
        if (this.dir == ScrollDirection.Vertical) {
            ...
            //手动计算
            this.content.y = this.content.height * this.content.anchorY - this.content.parent.anchorY * this.content.parent.height;
        }
        else {
            this.scrollview.scrollToRight();
        }
        this.changeAllItemRenderState();
        this.render();
        if (this.chat_auto_scrolling) {
            this.auto_scrolling = true;
        }
    ...
    scroll_to(index: number, time: number = 0) {
        ...
        const packItem = this.packItems[index];
        if (!packItem) {
            console.log("scroll_to, index out of range");
            return;
        }
        if (this.dir = ScrollDirection.Vertical) {
    ...
    private get_pack_item(index: number) {
        ...
        if (index < 0 || index >= this.packItems.length) {
            console.warn("get_pack_item, invalid index", index);
            return null;
        }
    ...
    private changeAllItemRenderState(state: boolean = true) {
        for (let i = 0; i < this.packItems.length; i++) {
            this.packItems[i].rendered = state;
        }
    }

    getUnRenderCount(): number {
        let count = 0;
        for (let i = 0; i < this.packItems.length; i++) {
            let packItem = this.packItems[i];
            if (packItem.item && packItem.rendered == false) {
                let pos: cc.Vec3 = packItem.item.node.position.add(this.content.position)
                if (this.dir == ScrollDirection.Vertical) {
                    if (pos.y > ((1 - this.scrollview.node.anchorY) * this.scrollview.node.height - this.scrollview.node.height + packItem.item.node.height)) {
                        packItem.rendered = true;
                    }
                }
                else {
                    if (pos.x < ((1 - this.scrollview.node.anchorX) * this.scrollview.node.width + packItem.item.node.width)) {
                        packItem.rendered = true;
                    }
                }
            }
            if (packItem.rendered == false) {
                count++;
            }
        }
        return count;
    ...
    destroy() {
        ...
        blade.ticker.offTick(this.onUpdate, this);
        // if(this.timer)
        // {
...
export type ScrollViewParams = {
    ...
    scroll_to_end_cb?: Function;       //滚动到尽头的回调
    auto_scrolling?: boolean;          //append时自动滚动到尽头
    moreNum?: number;
    chat_auto_scrolling?: boolean;
    render_cb?: Function;
}
