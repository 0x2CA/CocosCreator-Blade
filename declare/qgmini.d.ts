...
declare namespace qg {
    ...
    */
    ...
    interface BannerAd {
        ...
        style: {
            ...
            /** banner 广告组件的左上角纵坐标*/
            top: number,
            /** banner 广告组件的宽度。最小 300，最大至 屏幕宽度（屏幕宽度可以通过 qg.getSystemInfoSync() 获取）。*/
            width: number,
            /** banner 广告组件的高度*/
            height: number,
    ...
    /**
     * 插屏广告组件。插屏广告组件是一个原生组件，并且是一个全局单例。层级比上屏 Canvas 高，会覆盖在上屏 Canvas 上。插屏 广告组件默认是隐藏的，需要调用 InterstitialAd.show() 将其显示。
     */
    ...
    */
    ...
    /**
     * 通过 WebSocket 连接发送数据，需要先 qg.connectSocket，并在 qg.onSocketOpen 回调之后才能发送。
     */
    ...
    */
    ...
    /**
     * 通过 qg.login 接口获得的用户登录态拥有一定的时效性。用户越久未使用小程序，用户登录态越有可能失效。反之如果用户一直在使用小程序，则用户登录态一直保持有效。具体时效逻辑由微信维护，对开发者透明。开发者只需要调用 qg.checkSession 接口检测当前用户登录态是否有效。登录态过期后开发者可以再调用 qg.login 获取新的用户登录态。
     */
    ...
    interface AuthSetting {
        /** 是否授权用户信息，对应接口 qg.getUserInfo*/
        userInfo: boolean;
        /** 是否授权地理位置，对应接口 qg.getLocation*/
        userLocation: boolean;
        /** 是否授权微信运动步数，对应接口 qg.getWeRunData*/
        werun: boolean;
        /** 是否授权保存到相册 qg.saveImageToPhotosAlbum*/
    ...
    /** 获取用户过去三十天微信运动步数。需要先调用 qg.login 接口。步数信息会在用户主动进入小程序时更新。*/
    ...
    /** 监听加速度数据事件。频率根据 qg.startAccelerometer() 的 interval 参数。可使用 qg.stopAccelerometer() 停止监听。*/
    ...
    /**
     * 监听罗盘数据，频率：5 次/秒，接口调用后会自动开始监听，可使用 qg.stopCompass 停止监听。
    ...
    /**
     * 监听设备方向变化事件。频率根据 qg.startDeviceMotionListening() 的 interval 参数。可以使用 qg.stopDeviceMotionListening() 停止监听。
     */
...
//      */
//     qgBindCanvasTexture: (texture: number, canvas: qg.Canvas) => void
// }
