...
declare namespace qq {
    ...
    */
    ...
    /**
     * 创建 banner 广告组件。请通过 qq.getSystemInfoSync() 返回对象的 SDKVersion 判断基础库版本号 >= 2.6.0 后再使用该 API。
     * @param Option 
     */
    ...
    */
    ...
    /**
     * 通过 qq.login 接口获得的用户登录态拥有一定的时效性。用户越久未使用小程序，用户登录态越有可能失效。反之如果用户一直在使用小程序，则用户登录态一直保持有效。具体时效逻辑由微信维护，对开发者透明。开发者只需要调用 qq.checkSession 接口检测当前用户登录态是否有效。登录态过期后开发者可以再调用 qq.login 获取新的用户登录态。
     */
    ...
    interface AuthSetting {
        ...
        /** 是否授权微信运动步数，对应接口 qq.getWeRunData*/
        werun: boolean;
    ...
    /** 获取用户过去三十天微信运动步数。需要先调用 qq.login 接口。步数信息会在用户主动进入小程序时更新。*/
    ...
    /**
     * 监听罗盘数据，频率：5 次/秒，接口调用后会自动开始监听，可使用 qq.stopCompass 停止监听。
    ...
    /**
     * 监听设备方向变化事件。频率根据 qq.startDeviceMotionListening() 的 interval 参数。可以使用 qq.stopDeviceMotionListening() 停止监听。
     */
...
//      */
//     qqBindCanvasTexture: (texture: number, canvas: qq.Canvas) => void
// }
